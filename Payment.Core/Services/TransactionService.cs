...
namespace Payment.Core.Services
{
    public class TransactionService : ITransactionService
    {
        ...
        public async Task<ResponseDto<PaginationResult<IEnumerable<TransactionHistoryResponseDto>>>> GetTransactionHistoryOfTheLastMonthAsync(string walletId, int pageNumber)
        {
            var transactions = _unitOfWork.Transactions.GetTransactionHistory(walletId).OrderByDescending(t => t.CreatedAt).Where(x => x.CreatedAt >= DateTimeOffset.Now.AddDays(-30));
